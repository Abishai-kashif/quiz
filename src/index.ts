...
async function getQuiz(): Promise<Quiz[]> {
	try {
		...
		console.error(
			chalk.red("Error fetching quiz questions:", error.message)
		);
		process.exit(1);
	}
}

async function askQuestions(name: string, quizzes: Quiz[]) {
	let score = 0;
	...
	console.log(
		chalk.whiteBright.bold.italic(`
    \n\t\t\t${chalk.green.bold(name)}, your score is ${
			score < quizzes.length / 2
				? chalk.red(score)
				: chalk.greenBright(score)
		}/${chalk.greenBright(quizzes.length)}
  `)
	);
}
